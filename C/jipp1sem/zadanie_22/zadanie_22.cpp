...
int main(int argc, _TCHAR* argv[])
{
	...
	if (D < 0.0)
	{
		...
		double u1 = sqrt(fabs(pow(b / (2 * a), 2) - (c / a)));
		double u2 = 0 - sqrt(fabs(pow(b / (2 * a), 2) - (c / a)));

		//pierwiastki zespolone
		printf("Istnieja dwa pierwiaskti zespolone. Jeden to: %f + (%f)i\n\t,drugi: %f + (%f)i\n", r1, u1, r2, u2);
...
Zawiesza monitor dokd nie bdzie wprowadzony ^Z
exit(1)
===============================================================================*/
{
	int ch;
